...
import { FoodAllergy, AllergySeverity, COMMON_ALLERGIES, ALLERGY_SEVERITIES } from '../types/index';
...
export const FoodAllergies = ({
  ...
  const [newAllergySeverity, setNewAllergySeverity] = useState<AllergySeverity>('mild-intolerance');
  const [editingId, setEditingId] = useState<string | null>(null);
  const [editName, setEditName] = useState('');
  const [editSeverity, setEditSeverity] = useState<AllergySeverity>('mild-intolerance');
  ...
  const availableCommonAllergies = COMMON_ALLERGIES.filter(
    commonAllergy => !allergies.some(allergy => allergy.name === commonAllergy)
  );
  ...
  return (
    <div className="card border-red-200 bg-red-50">
      ...
      <div className="bg-red-100 border border-red-300 rounded-lg p-4 mb-4">
        <p className="text-red-800 font-medium mb-2">⚠️ Critical Medical Information</p>
        <p className="text-red-700 text-sm">
          This section contains vital health information. Please be accurate and thorough when adding allergies and intolerances.
        </p>
      </div>
      ...
      {availableCommonAllergies.length > 0 && (
        <div className="mb-6">
          <h3 className="text-lg font-medium text-gray-800 mb-3">Quick Add Common Allergies</h3>
          ...
          <div className="flex items-center gap-3 mb-4">
            <label htmlFor="common-allergy-severity" className="text-sm font-medium text-gray-700">
      ...
      <div className="mb-6 p-4 bg-white rounded-lg border border-red-200">
        <h3 className="text-lg font-medium text-gray-800 mb-3">Add Custom Allergy/Intolerance</h3>
        <div className="flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="sm:w-48">
              <label htmlFor="new-allergy-severity" className="block text-sm font-medium text-gray-700 mb-1">
                Severity Level
              </label>
              <select
            ...
            <ul className="space-y-1 ml-4">
              {ALLERGY_SEVERITIES.map((severity) => (
                <li key={severity.value} className="flex items-center gap-2">
                  <span className={`px-2 py-1 text-xs rounded-full border ${getSeverityColor(severity.value)}`}>
      ...
      {allergies.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-sm">Add any known food allergies or intolerances above.</p>
        </div>
      ) : (
            ...
            >
              {editingId === allergy.id ? (
                <div className="flex flex-1 gap-3 items-center flex-wrap">
                  ...
                  <select
                    value={editSeverity}
                    onChange={(e) => setEditSeverity(e.target.value as AllergySeverity)}
                    className="input-field w-48"
                  >
                    {ALLERGY_SEVERITIES.map((severity) => (
                      <option key={severity.value} value={severity.value}>
                ...
                <>
                  <div className="flex-1">
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className="font-medium text-gray-800 text-lg">{allergy.name}</span>
                      {allergy.isCommon && (
                        <span className="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded-full">
                          Common Allergy
                        </span>
                      )}
                      <span className={`px-3 py-1 text-sm rounded-full border font-medium ${getSeverityColor(allergy.severity)}`}>
                        <span className="mr-1">{getSeverityIcon(allergy.severity)}</span>
                        {ALLERGY_SEVERITIES.find(s => s.value === allergy.severity)?.label}
                      </span>
                    </div>
                    <p className="text-sm text-gray-600 mt-1">
                      {ALLERGY_SEVERITIES.find(s => s.value === allergy.severity)?.description}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    <button
