...
import { MealPreferences, FoodItem, DislikedFood, FoodAllergy, FoodCategory, AllergySeverity, DislikeSeverity } from '../types/index';
...
export const useMealPreferences = () => {
  ...
  // Favorite Foods
  const addFavoriteFood = useCallback((name: string, category?: FoodCategory) => {
  ...
  const updateFavoriteFood = useCallback((id: string, name: string, category?: FoodCategory) => {
    setPreferences(prev => ({
      ...prev,
  ...
  // Disliked Foods
  const addDislikedFood = useCallback((name: string, severity: DislikeSeverity, category?: FoodCategory) => {
  ...
  const updateDislikedFood = useCallback((id: string, name: string, severity: DislikeSeverity, category?: FoodCategory) => {
    setPreferences(prev => ({
      ...prev,
      dislikedFood: prev.dislikedFood.map(food =>
        food.id === id ? { ...food, name: name.trim(), severity, category } : food
      ),
    }));
  }, []);
  ...
  // Allergies
  const addAllergy = useCallback((name: string, severity: AllergySeverity, isCommon = false) => {
  ...
  const updateAllergy = useCallback((id: string, name: string, severity: AllergySeverity) => {
    setPreferences(prev => ({
      ...prev,
      allergies: prev.allergies.map(allergy =>
        allergy.id === id ? { ...allergy, name: name.trim(), severity } : allergy
      ),
    }));
  }, []);
